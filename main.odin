...
init_application :: proc() -> bool {
    if sdl.Init({.VIDEO, .AUDIO}) < 0 {
        fmt.println("sdl could not initialize! sdl Error: %v", sdl.GetError())
        return false
    }

    if sdl_mixer.OpenAudio(44100, sdl.AUDIO_S16SYS, 2, 512) < 0 {
        fmt.println("Unable to open audio: %v\n", sdl.GetError())
        return false
    }

    if sdl_image.Init({.PNG}) == nil {
        fmt.println("sdl.image could not initialize! sdl.mage Error: %v\n", sdl_image.GetError())
        return false
    }
    ...
    if window == nil {
        fmt.println("Window could not be created! SDL Error: %v\n", sdl.GetError())
        return false
    }

    renderer = sdl.CreateRenderer(window, -1, {.ACCELERATED, .PRESENTVSYNC})
    if renderer == nil {
        fmt.println("Renderer could not be created! SDL Error: %v\n", sdl.GetError())
        return false
    }

    gon.default_log_proc = proc(format: string, args: ..any, loc := #caller_location) {
    ...
    tiles_texture    = load_texture(renderer, "data/gfx/tiles2.png" ) or_return
    ...
    
    {
        ...
        tile_size = 16
        tileset_size = { i32(tiles_texture.width) / tile_size, i32(tiles_texture.height) / tile_size }
    }
...
init_game :: proc() -> bool {
    ...
    //     set_all_tiles_on_tilemap(&s.tilemap, { id = 221 })
    // }
